# Product Proposal — PrivVault

**Program:** New Moon to Full: Monthly Moonshots on Midnight
**Project:** PrivVault — Private Access & Credential Verification
...
PrivVault is designed to make that verification private.

## Product

**PrivVault** is a reusable private access and credential verification layer built on Midnight.
...
PrivVault is intended to become infrastructure that other Midnight developers can reuse instead of rebuilding private access logic for every application.

The MVP will provide a simple **PrivVault SDK/API** for developers to:
...
The SDK will handle the PrivVault verification flow, including interacting with the credential/access contract and returning the verification result to the integrating application.

This allows another dApp to use PrivVault as a private access layer without implementing its own credential verification system from scratch.

## Sample dApp — PrivVault Developer Portal

To demonstrate that PrivVault works as reusable infrastructure, I will build a small Midnight dApp that integrates the PrivVault SDK.
...
**Issuer creates credential → User receives credential → User opens protected dApp → dApp requests PrivVault verification → Zero Knowledge proof verifies eligibility → PrivVault returns verification result → Protected feature unlocks**
...
This sample dApp will demonstrate the full integration rather than showing PrivVault only as an isolated proof-of-concept.

## Why Midnight

PrivVault needs users to prove facts about private credentials without exposing the credentials themselves.
...
With PrivVault on Midnight, the application can verify:
...
That privacy boundary is the main reason PrivVault is being built on Midnight.
...
6. Build the PrivVault SDK/API for external dApp integration.
7. Build a sample Midnight Developer Portal protected using the PrivVault SDK.
...
PrivVault should not be only a single credential-gated application.
